...
describe Flame::Application do
	...
	describe '.require_dirs' do
		...
		let(:files) { Dir[File.join(__dir__, 'require_dirs/**/*')] }

		it 'requires all wanted files' do
			files.reject! do |file|
				File.executable?(file) || file.match?(%r{lib/\w+/spec/})
			end

			files.each do |file|
				expect(require(file)).to be false
			end
		end

		it 'does not require executable files' do
			files
				.select { |file| File.file?(file) && File.executable?(file) }
				.each do |file|
					expect(require(file)).to be true
				end
		end

		it 'does not require ignored files' do
			files
